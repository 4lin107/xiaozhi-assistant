...
class SecurityManager:
    ...
    def sanitize_input(self, input_data):
        if isinstance(input_data, str):
            for c in ['\x00', '\n', '\r', '\t']: input_data = input_data.replace(c, '')
        return input_data
    ...
    def has_permission(self, cmd):
        if cmd not in self.COMMAND_PERMISSIONS: return True
    ...
    def set_user_permission(self, p):
        if p in self.PERMISSION_LEVELS: self.current_user_permission = p; return True
        return False
    
    def get_user_permission(self): return self.current_user_permission
    def get_user_role(self, uid): return self.user_roles.get(uid, 'guest')

security_manager = None
def get_security_manager():
    global security_manager
    if not security_manager: security_manager = SecurityManager()
